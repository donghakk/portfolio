...
for lp in data['projects'][0]['videos']:
    ...
    cards.append(f'''<figure class="lp-preview"><div class="video-heading"><h3>{title}</h3><a href="{url}" target="_blank" rel="noopener">운영 페이지 ↗</a></div><iframe class="lp-frame" src="{url}" title="셀프애드 {title} 실제 운영 페이지" width="390" height="720" loading="lazy" referrerpolicy="strict-origin-when-cross-origin" sandbox="allow-scripts allow-same-origin allow-forms allow-popups allow-popups-to-escape-sandbox"></iframe><figcaption>프레임 안에서 스크롤해 LP를 살펴보세요.</figcaption></figure>''')
